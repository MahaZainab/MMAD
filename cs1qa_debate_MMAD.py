# =============================================================================
# CS1QA Multi-Model Evaluator  -  Teacher-Guided Theory-of-Mind Debate
#
# Dataset : CS1QA  (each example has: code, question, questionType, answer)
#           The task is to answer a natural-language question about a student's
#           introductory CS code snippet (e.g. explaining an algorithm,
#           answering a task question, describing what the code does).
#           questionType indicates the category (e.g. "code_explain", "task").
#
# Models:
...
# CSV columns
# -----------
# code, question, question_type, gold_answer,
...
DEFAULT_RESULTS_JSON   = "cs1qa_debate_results.json"
DEFAULT_RESULTS_CSV    = "cs1qa_debate_results.csv"
...
# Prompt builders
# ---------------------------------------------------------------------------
def build_contestant_prompt(code: str, question: str, question_type: str) -> str:
    """Phase 0: independent QA for CS1QA, no context from other agents."""
    return f"""You are given a Python code snippet written by an introductory CS student \
and a question about it. The question type is: "{question_type}".
...
- For yes/no questions, answer "Yes" or "No" followed by a brief justification.
- For explanation/algorithm questions, give a short but complete explanation.
- Do not repeat the question or the code in your answer.
...
def build_teacher_prompt(
    ...
    """
    Teacher (Theory-of-Mind) prompt for CS1QA.
...
"""
    ...
    return f"""You are an expert CS1 teaching assistant and Theory-of-Mind reasoner \
participating in Round {round_num} of a {NUM_DEBATE_ROUNDS}-round multi-agent debate.

Three small language model agents (M, P, Q) are each trying to answer a question \
(type: "{question_type}") about an introductory CS student's Python code snippet. \
...
Do NOT copy this answer verbatim into your guidance. Instead, use it to:
  ...
  - Steer incorrect agents firmly toward the right direction.
...
- If an agent is WRONG: Tell them clearly which part of the code or reasoning led
  them astray. Guide them firmly toward the correct answer without quoting it word-for-word.
...
def build_debate_prompt(
    ...
    """
    Student debate prompt for CS1QA - works for both pure-debate and teacher-guided rounds.
    """
    peer_block = ""
...
Round {round_num} of a {NUM_DEBATE_ROUNDS}-round multi-agent debate for CS1 code question answering.

You will receive:
1. The Python code snippet (written by an introductory CS student) and the question about it.
...
def build_judge_prompt(code: str, question: str, gold: str, prediction: str) -> str:
    return f"""You are an automated judge for a CS1 code question-answering task.
...
- Minor wording differences are ACCEPTABLE: paraphrases that preserve the core meaning are CORRECT.
- For yes/no questions, any clear affirmative counts as Yes and any clear negative counts as No.
- Partial answers that capture the key fact or idea are CORRECT.
...
def load_dataset(path: str) -> List[Dict]:
    """
    Reads a CS1QA JSON dataset (list of dicts with keys: code, question, questionType, answer).
    Normalises to internal keys: code, input (=question), question_type, output (=answer).
        ...
        with open(path, newline="", encoding="utf-8") as f:
            ...
            for i, row in enumerate(reader):
                rows.append({
                    ...
                    "question_type": row.get("questionType", row.get("question_type", "")),
    ...
    # Normalise CS1QA JSON fields to internal convention
    normalised = []
    for i, ex in enumerate(data):
        normalised.append({
            "id":            ex.get("id", f"idx_{i}"),
            "code":          ex.get("code", ""),
            "input":         ex.get("question", ex.get("input", "")),         # CS1QA uses "question"
            "question_type": ex.get("questionType", ex.get("question_type", "")),  # CS1QA extra field
            "output":        ex.get("answer", ex.get("output", "")),          # CS1QA uses "answer"
        })
    return normalised
...
def run_contestant(
    ...
    for idx, ex in enumerate(dataset):
        ...
        prompt = build_contestant_prompt(code, inp, qtype)
...
def run_teacher(
    ...
    """
    For each example, the teacher is privately given the gold answer and each
    agent's latest answer, then produces personalised guidance per agent.
    ...
    for idx, ex in enumerate(dataset):
        ...
        inp         = ex.get("input",         "")
        qtype       = ex.get("question_type", "")
        gold_answer = ex.get("output",        "")   # private gold answer for teacher only
        ex_id       = ex.get("id",            f"idx_{idx}")
        ...
        prompt = build_teacher_prompt(code, inp, qtype, gold_answer, round_num, agent_responses)
        ...
        fallback = (
            "Re-examine the code step by step. "
            "Make sure you are correctly tracing how variables change and what each function does."
        )
...
def write_outputs(
    ...
    for idx, ex in enumerate(dataset):
        row = {
            ...
            "question_type": ex.get("question_type", ""),   # CS1QA extra field
            "gold_answer":   ex.get("output",        ""),   # display as "gold_answer" in output
        }
    ...
    fieldnames = ["code", "question", "question_type", "gold_answer"]
...
def print_summary(
    ...
    print("\n" + sep)
    print("SUMMARY  (teacher-guided ToM debate  -  CS1QA)")
    print(sep)
...
def main():
    ...
    print(f"Dataset        : {dataset_path}  [CS1QA]")
